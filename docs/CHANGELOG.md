...
- [修复] A 股基本面财报宽表解析（stock_financial_abstract 指标行×报告期列），补全营收/归母净利/ROE/毛利率/每股净资产/资产负债率等字段。
- [修复] 业绩预告/快报接口传参（date）与东财分红列名映射，修复分红每股派息解析。
- [改进] A 股资金流多源 fallback（东财→新浪→同花顺/腾讯）并扩展超大单/大单/小单/主力净占比字段，东财不可达时不再显示"资金流数据缺失"。
- [新功能] A 股融资融券明细（融资余额/融资买入额/融券余量）并入资金流上下文。
- [新功能] A 股个股新闻（东方财富直连）与公司公告（巨潮资讯直连）接入日报报告，无第三方搜索 key 也能产出新闻与公告。
- [改进] 基本面聚合阶段预算默认 8s→12s，保证估值行情(多源 fallback)之后资金流多源 fallback 与融资融券明细仍有足额预算。
